...
Install for voice: pip install SpeechRecognition pyaudio pyttsx3
"""
...
STATUS_MSG = "Station reference mode: routes follow taught walkways | Click 🎤 to speak"
...
def listen_voice_command():
    """Listen once from the microphone, convert speech to text, then run navigation."""
    global STATUS_MSG

    if not SPEECH_OK:
        STATUS_MSG = "SpeechRecognition not installed. Run: pip install SpeechRecognition pyaudio"
        return

    def worker():
        global STATUS_MSG
        try:
            STATUS_MSG = "Listening... say: navigate from Glenferrie Station to EN"
            ...
            STATUS_MSG = "Recognising voice command..."
            text = recognizer.recognize_google(audio)
            command_var.set(text)
            STATUS_MSG = f"Heard: {text}"

            # Tkinter UI updates should be called from the main thread.
            root.after(0, handle_command)

        except sr.WaitTimeoutError:
            STATUS_MSG = "No speech detected. Try again."
        except sr.UnknownValueError:
            STATUS_MSG = "Could not understand. Try: navigate from station to EN"
        except sr.RequestError:
            STATUS_MSG = "Speech service unavailable. Check internet connection."
        except Exception as e:
            STATUS_MSG = f"Mic error: {e}"
...
def on_click(e):
    ...
    # Microphone button at bottom-right of left panel
    if PANEL_W-82 <= e.x <= PANEL_W-4 and H-52 <= e.y <= H:
        listen_voice_command()
        return

    # Bottom-left area advances the instruction step for demo/testing
